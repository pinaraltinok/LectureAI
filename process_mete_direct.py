...
def start_transcription(video_url):
    print(f"[>>] AssemblyAI transkript baslatiliyor (Direct URL)...")
...
def wait_for_transcription(transcript_id):
    ...
    headers = {"authorization": ASSEMBLYAI_API_KEY}
    while True:
        resp = httpx.get(f"https://api.assemblyai.com/v2/transcript/{transcript_id}", headers=headers, timeout=30)
        data = resp.json()
        status = data["status"]
        if status == "completed":
...
if __name__ == "__main__":
    ...
    print("\nSimdi biyometrik eslestirme ve rapor adimina gecebiliriz.")
