...
const StudentSurvey = () => {
  ...
  const { courseName, instructor } = location.state || { courseName: 'Python Programming', instructor: 'Zehra Bozkurt' }

  const [isSubmitted, setIsSubmitted] = useState(false)
  const [comment, setComment] = useState('')
  const [ratings, setRatings] = useState({ clarity: 0, support: 0, energy: 0, satisfaction: 0 })

  const canSubmit = ratings.clarity && ratings.support && ratings.energy && ratings.satisfaction
  ...
  if (isSubmitted) {
    return (
      <div className="report-card-internal" style={{ maxWidth: '400px', margin: '100px auto', textAlign: 'center', padding: '3rem', border: 'none', background: 'linear-gradient(135deg, #6366f1, #a855f7)', color: 'white' }}>
        ...
        <h1 style={{ fontSize: '1.75rem', fontWeight: 800, marginBottom: '0.5rem' }}>Success!</h1>
        <p style={{ opacity: 0.9, fontSize: '0.95rem', marginBottom: '2.5rem' }}>
          Feedback sent. You are a rockstar!
        </p>
    ...
    <div style={{
      ...
      <div style={{ maxWidth: '740px', margin: '0 auto', animation: 'fadeIn 0.5s ease', position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem', alignItems: 'center' }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, #ef4444, #f43f5e)', color: '#fff', padding: '4px 12px', borderRadius: '100px', fontSize: '9px', fontWeight: 800, boxShadow: '0 4px 10px rgba(239, 68, 68, 0.2)' }}>SECURE MODE ACTIVE</div>
        </div>
        ...
        <div className="report-card-internal" style={{
          ...
          <div style={{
            ...
            <h2 style={{ fontSize: '1.8rem', fontWeight: 800, letterSpacing: '-0.03em', marginBottom: '0.2rem' }}>{courseName}</h2>
            <p style={{ opacity: 0.9, fontSize: '1rem', fontWeight: 600 }}>Instruction Assessment for {instructor}</p>
          ...
          <div style={{ padding: '2.5rem 2rem' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              <SurveySection title="Technical Clarity & Explanation" value={ratings.clarity} onChange={(v) => setRatings({ ...ratings, clarity: v })} />
              <SurveySection title="Communication & Support Speed" value={ratings.support} onChange={(v) => setRatings({ ...ratings, support: v })} />
              <SurveySection title="Class Energy & Motivation Levels" value={ratings.energy} onChange={(v) => setRatings({ ...ratings, energy: v })} />
              <SurveySection title="Overall Recommendation Score" value={ratings.satisfaction} onChange={(v) => setRatings({ ...ratings, satisfaction: v })} />
            </div>
            ...
            <div style={{ marginTop: '3rem', borderTop: '1px solid #f1f5f9', paddingTop: '2.5rem' }}>
              <label style={{ fontSize: '11px', fontWeight: 800, color: '#64748b', display: 'block', marginBottom: '1rem', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                Private feedback for the instructor
              </label>
              <textarea
